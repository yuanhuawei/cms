...
namespace MaiDarServer.BackgroundPages.Controls
{
...
SELECT * FROM (
    SELECT TOP {recsToRetrieve} * FROM (
        ...
        private int GetQueryVirtualCount()
        {
            ...
            var recCount = MaiDarDataProvider.DatabaseDao.GetIntResult(WebConfigUtils.ConnectionString, cmdText);
            //            SqlConnection conn = new SqlConnection(ConnectionString);
            //            SqlCommand cmd = new SqlCommand(cmdText, conn);
            //IDbConnection conn = SqlUtils.GetIDbConnection(MaiDarDataProvider.ADOType, ConnectionString);
            //IDbCommand cmd = SqlUtils.GetIDbCommand(MaiDarDataProvider.ADOType);
            //cmd.Connection = conn;
            //cmd.CommandText = cmdText;
