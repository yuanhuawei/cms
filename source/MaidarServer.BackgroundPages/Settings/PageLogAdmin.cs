...
namespace MaiDarServer.BackgroundPages.Settings
{
    public class PageLogAdmin : BasePage
    {
		...
		public void Page_Load(object sender, EventArgs e)
        {
            ...
            SpContents.SelectCommand = !Body.IsQueryExists("Keyword") ? MaiDarDataProvider.LogDao.GetSelectCommend() : MaiDarDataProvider.LogDao.GetSelectCommend(Body.GetQueryString("UserName"), Body.GetQueryString("Keyword"), Body.GetQueryString("DateFrom"), Body.GetQueryString("DateTo"));
			...
			if(!IsPostBack)
			{
                ...
                if (Body.IsQueryExists("Delete"))
                {
                    ...
                    try
                    {
                        MaiDarDataProvider.LogDao.Delete(arraylist);
                ...
                else if (Body.IsQueryExists("DeleteAll"))
                {
                    try
                    {
                        MaiDarDataProvider.LogDao.DeleteAll();
                        SuccessDeleteMessage();
                    }
                    catch (Exception ex)
                    {
