    ...
    public class PageAuxiliaryTableAdd : BasePageCms
    {
        ...
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    var info = MaiDarDataProvider.TableCollectionDao.GetAuxiliaryTableInfo(enName);
                    if (info != null)
                    {
        ...
        public override void Submit_OnClick(object sender, EventArgs e)
        {
            if (Page.IsPostBack && Page.IsValid)
            {
                if (Body.IsQueryExists("ENName"))
                {
                    ...
                    var info = MaiDarDataProvider.TableCollectionDao.GetAuxiliaryTableInfo(tableEnName);
                    info.TableCnName = TableCNName.Text;
                    info.Description = Description.Text;
                    try
                    {
                        MaiDarDataProvider.TableCollectionDao.Update(info);
                ...
                {
                    var tableEnNameList = MaiDarDataProvider.TableCollectionDao.GetTableEnNameList();
                    if (tableEnNameList.IndexOf(TableENName.Text) != -1)
                    {
                        FailMessage("辅助表添加失败，辅助表标识已存在！");
                    }
                    else if (MaiDarDataProvider.DatabaseDao.IsTableExists(TableENName.Text))
                    {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            MaiDarDataProvider.TableCollectionDao.Insert(info);
