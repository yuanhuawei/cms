	...
	public class PageAdministratorAdd : BasePage
    {
		...
		public void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                {
                    var adminInfo = MaiDarDataProvider.AdministratorDao.GetByUserName(userName);
                    if (adminInfo != null)
                    {
        ...
        public override void Submit_OnClick(object sender, EventArgs e)
        {
            if (Page.IsPostBack && Page.IsValid)
            {
                if (string.IsNullOrEmpty(userName))
                {
                    ...
                    if (!string.IsNullOrEmpty(MaiDarDataProvider.AdministratorDao.GetUserNameByEmail(tbEmail.Text)))
                    {
                        FailMessage("管理员添加失败，邮箱地址已存在");
                        return;
                    }

                    if (!string.IsNullOrEmpty(MaiDarDataProvider.AdministratorDao.GetUserNameByMobile(tbMobile.Text)))
                    {
                ...
                else
                {
                    ...
                    if (adminInfo.Email != tbEmail.Text && !string.IsNullOrEmpty(MaiDarDataProvider.AdministratorDao.GetUserNameByEmail(tbEmail.Text)))
                    {
                        FailMessage("管理员设置失败，邮箱地址已存在");
                        return;
                    }

                    if (adminInfo.Mobile != tbMobile.Text && !string.IsNullOrEmpty(MaiDarDataProvider.AdministratorDao.GetUserNameByMobile(adminInfo.Mobile)))
                    {
                    ...
                    MaiDarDataProvider.AdministratorDao.Update(adminInfo);
