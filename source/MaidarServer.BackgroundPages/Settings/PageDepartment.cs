...
namespace MaiDarServer.BackgroundPages.Settings
{
    public class PageDepartment : BasePage
    {
        ...
        public static string GetRedirectUrl(int currentDepartmentId)
        {
            ...
            {
                return PageUtils.GetSettingsUrl(nameof(PageDepartment), new NameValueCollection
                {
                    {"CurrentDepartmentID", currentDepartmentId.ToString() }
                });
            }
            return PageUtils.GetSettingsUrl(nameof(PageDepartment), null);
        }
        ...
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Body.IsQueryExists("Delete") && Body.IsQueryExists("DepartmentIDCollection"))
            {
                ...
                {
                    MaiDarDataProvider.DepartmentDao.Delete(departmentId);
                }
            ...
            else if (Body.IsQueryExists("DepartmentID") && (Body.IsQueryExists("Subtract") || Body.IsQueryExists("Add")))
            {
                ...
                MaiDarDataProvider.DepartmentDao.UpdateTaxis(departmentId, isSubtract);
            ...
            if (!IsPostBack)
            {
                BreadCrumbSettings("所属部门管理", AppManager.Permissions.Settings.AdminManagement);
                ...
                Delete.Attributes.Add("onclick", PageUtils.GetRedirectStringWithCheckBoxValueAndAlert(PageUtils.GetSettingsUrl(nameof(PageDepartment), new NameValueCollection
                {
                    {"Delete", "True" }
        ...
        public void BindGrid()
        {
            try
            {
                rptContents.DataSource = MaiDarDataProvider.DepartmentDao.GetDepartmentIdListByParentId(0);
...
";
            ...
            else if (loadingType == EDepartmentLoadingType.ContentList)
            {
                ...
                var urlUp = PageUtils.GetSettingsUrl(nameof(PageDepartment), new NameValueCollection
                {
                    {"Subtract", "True"},
                ...
                var urlDown = PageUtils.GetSettingsUrl(nameof(PageDepartment), new NameValueCollection
                {
                    {"Add", "True"},
                    {"DepartmentID", departmentInfo.DepartmentId.ToString()}
                });
