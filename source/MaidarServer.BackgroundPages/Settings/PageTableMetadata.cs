...
namespace MaiDarServer.BackgroundPages.Settings
{
    public class PageTableMetadata : BasePageCms
    {
        ...
        {
            return PageUtils.GetSettingsUrl(nameof(PageTableMetadata), new NameValueCollection
            {
                {"ENName", tableName},
        ...
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            var tableInfo = MaiDarDataProvider.TableCollectionDao.GetAuxiliaryTableInfo(_tableName);
            ...
            if (Body.IsQueryExists("Delete"))
            {
                ...
                try
                {
                    var tableMetadataInfo = MaiDarDataProvider.TableMetadataDao.GetTableMetadataInfo(tableMetadataId);
                    MaiDarDataProvider.TableMetadataDao.Delete(tableMetadataId);
            ...
            else if (Body.IsQueryExists("CreateDB"))
            {
                try
                {
                    MaiDarDataProvider.TableMetadataDao.CreateAuxiliaryTable(_tableName);
                ...
                catch (Exception ex)
                {
                    ...
                    var sqlString = MaiDarDataProvider.TableMetadataDao.GetCreateAuxiliaryTableSqlString(_tableName);
            ...
            else if (Body.IsQueryExists("DeleteDB"))
            {
                try
                {
                    MaiDarDataProvider.TableMetadataDao.DeleteAuxiliaryTable(_tableName);
                    tableInfo.IsChangedAfterCreatedInDb = false;
            ...
            else if (Body.IsQueryExists("ReCreateDB"))
            {
                try
                {
                    MaiDarDataProvider.TableMetadataDao.ReCreateAuxiliaryTable(_tableName, tableInfo.AuxiliaryTableType);
            ...
            _tableIsRealCreated = MaiDarDataProvider.DatabaseDao.IsTableExists(_tableName);

            _usedNum = MaiDarDataProvider.TableCollectionDao.GetTableUsedNum(_tableName, tableInfo.AuxiliaryTableType);
            ...
            if (!IsPostBack)
            {
                ...
                DgContents.DataSource = MaiDarDataProvider.TableMetadataDao.GetDataSource(_tableName);
        ...
        private void SetTaxis()
        {
            ...
            {
                case "UP":
                    MaiDarDataProvider.TableMetadataDao.TaxisDown(tableMetadataId, _tableName);
                    break;
                case "DOWN":
                    MaiDarDataProvider.TableMetadataDao.TaxisUp(tableMetadataId, _tableName);
                    break;
            }
            SuccessMessage("排序成功！");
        ...
        void dgContents_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (IsSystem(isSystem))
                {
                    ...
                    {
                        upLinkButton.NavigateUrl = PageUtils.GetSettingsUrl(nameof(PageTableMetadata),
                    ...
                    {
                        downLinkButton.NavigateUrl = PageUtils.GetSettingsUrl(nameof(PageTableMetadata),
                            new NameValueCollection
                            {
                                {"PublishmentSystemID", PublishmentSystemId.ToString()},
                                {"SetTaxis", "True"},
        ...
        public void SyncTableButton_OnClick(object sender, EventArgs e)
        {
            ...
            try
            {
                MaiDarDataProvider.TableMetadataDao.SyncTable(_tableName);
