...
namespace MaiDarServer.BackgroundPages.Settings
{
	public class ModalAdminPassword : BasePage
	{
		...
		public void Page_Load(object sender, EventArgs e)
        {
			...
			if (!IsPostBack)
			{
                if (!string.IsNullOrEmpty(_userName) && MaiDarDataProvider.AdministratorDao.IsUserNameExists(_userName))
                {
        ...
        public override void Submit_OnClick(object sender, EventArgs e)
        {
			if (IsPostBack && IsValid)
			{
				...
				{
                    string errorMessage;
                    if (!MaiDarDataProvider.AdministratorDao.ChangePassword(_userName, Password.Text, out errorMessage))
                    {
