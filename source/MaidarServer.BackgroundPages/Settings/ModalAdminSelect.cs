...
namespace MaiDarServer.BackgroundPages.Settings
{
	public class ModalAdminSelect : BasePage
    {
			...
			if (!IsPostBack)
			{
                ...
                else if (Body.IsQueryExists("departmentID"))
                {
                    if (_departmentId > 0)
                    {
                        ...
                        rptUser.DataSource = MaiDarDataProvider.AdministratorDao.GetUserNameArrayList(_departmentId, false);
        ...
        public void BindGrid()
        {
            try
            {
                rptDepartment.DataSource = MaiDarDataProvider.DepartmentDao.GetDepartmentIdListByParentId(0);
