    ...
    public class PageCreateTaskLog : BasePage
    {
		...
		public void Page_Load(object sender, EventArgs e)
        {
			...
			if(!IsPostBack)
            {
                ...
                BtnDelete.Attributes.Add("onclick", PageUtils.GetRedirectStringWithCheckBoxValueAndAlert(PageUtils.GetSettingsUrl(nameof(PageCreateTaskLog), new NameValueCollection
                {
                    {"Delete", "True"}
                }), "IDCollection", "IDCollection", "请选择需要删除的日志！", "此操作将删除所选日志，确认吗？"));

                BtnDeleteAll.Attributes.Add("onclick", PageUtils.GetRedirectStringWithConfirm(PageUtils.GetSettingsUrl(nameof(PageCreateTaskLog), new NameValueCollection
                {
                    {"DeleteAll", "True"}
                }), "此操作将删除所有日志信息，确定吗？"));
        ...
        private string PageUrl => PageUtils.GetSettingsUrl(nameof(PageCreateTaskLog), new NameValueCollection
        {
