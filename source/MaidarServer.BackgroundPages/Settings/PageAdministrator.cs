        ...
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Body.IsQueryExists("Delete"))
            {
                ...
                try
                {
                    ...
                    {
                        MaiDarDataProvider.AdministratorDao.Delete(userName);
                    }
            ...
            else if (Body.IsQueryExists("Lock"))
            {
                ...
                try
                {
                    ...
                    MaiDarDataProvider.AdministratorDao.Lock(userNameList);
            ...
            else if (Body.IsQueryExists("UnLock"))
            {
                ...
                {
                    var userNameList = TranslateUtils.StringCollectionToStringList(userNameCollection);
                    MaiDarDataProvider.AdministratorDao.UnLock(userNameList);
            ...
            if (string.IsNullOrEmpty(Body.GetQueryString("PageNum")))
            {
                ...
                spContents.SelectCommand = MaiDarDataProvider.AdministratorDao.GetSelectCommand(permissioins.IsConsoleAdministrator, Body.AdminName, _departmentId);
                spContents.SortField = MaiDarDataProvider.AdministratorDao.GetSortFieldName();
                spContents.SortMode = SortMode.ASC;
            }
            else
            {
                spContents.ItemsPerPage = Body.GetQueryInt("PageNum") == 0 ? StringUtils.Constants.PageSize : Body.GetQueryInt("PageNum");
                spContents.SelectCommand = MaiDarDataProvider.AdministratorDao.GetSelectCommand(Body.GetQueryString("Keyword"), Body.GetQueryString("RoleName"), Body.GetQueryInt("LastActivityDate"), permissioins.IsConsoleAdministrator, Body.AdminName, _departmentId, Body.GetQueryInt("AreaID"));
            ...
            var allRoles = permissioins.IsConsoleAdministrator ? MaiDarDataProvider.RoleDao.GetAllRoles() : MaiDarDataProvider.RoleDao.GetAllRolesByCreatorUserName(Body.AdminName);
