	...
	public class ModalDepartmentSelect : BasePage
    {
	    ...
	    public static string GetShowPopWinString(int projectId)
	    {
	        return PageUtils.GetOpenWindowString("设置分类",
	            PageUtils.GetSettingsUrl(nameof(ModalDepartmentSelect), new NameValueCollection
	            {
		...
		{
		    _additional.Add("UrlFormatString", PageUtils.GetSettingsUrl(nameof(ModalDepartmentSelect), null));
        ...
        public void BindGrid()
        {
            try
            {
                rptCategory.DataSource = MaiDarDataProvider.DepartmentDao.GetDepartmentIdListByParentId(0);
