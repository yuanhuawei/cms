...
namespace MaiDarServer.BackgroundPages.Settings
{
    public class PageArea : BasePage
    {
        ...
        public static string GetRedirectUrl(int currentAreaId)
        {
            if (currentAreaId > 0)
            {
                return PageUtils.GetSettingsUrl(nameof(PageArea), new NameValueCollection
                {
                    {"CurrentAreaID", currentAreaId.ToString()}
                });
            }
            return PageUtils.GetSettingsUrl(nameof(PageArea), null);
        }
        ...
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Body.IsQueryExists("Delete") && Body.IsQueryExists("AreaIDCollection"))
            {
                ...
                {
                    MaiDarDataProvider.AreaDao.Delete(areaId);
                }
            ...
            else if (Body.IsQueryExists("AreaID") && (Body.IsQueryExists("Subtract") || Body.IsQueryExists("Add")))
            {
                ...
                MaiDarDataProvider.AreaDao.UpdateTaxis(areaId, isSubtract);
            ...
            if (!IsPostBack)
            {
                ...
                var urlDelete = PageUtils.GetSettingsUrl(nameof(PageArea), new NameValueCollection
                {
                    {"Delete", "True"}
                });
        ...
        public void BindGrid()
        {
            try
            {
                rptContents.DataSource = MaiDarDataProvider.AreaDao.GetAreaIdListByParentId(0);
        ...
        public static string GetAreaRowHtml(AreaInfo areaInfo, EAreaLoadingType loadingType, NameValueCollection additional)
        {
            ...
            if (loadingType == EAreaLoadingType.Management)
            {
                ...
                var urlUp = PageUtils.GetSettingsUrl(nameof(PageArea), new NameValueCollection
                {
                    {"Subtract", "True"},
                ...
                var urlDown = PageUtils.GetSettingsUrl(nameof(PageArea), new NameValueCollection
                {
                    {"Add", "True"},
                    {"AreaID", areaInfo.AreaId.ToString()}
                });
