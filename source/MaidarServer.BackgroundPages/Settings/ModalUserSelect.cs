...
namespace MaiDarServer.BackgroundPages.Settings
{
    public class ModalUserSelect : BasePage
    {
        ...
        {
            return PageUtils.GetOpenWindowString("选择用户", PageUtils.GetSettingsUrl(nameof(ModalUserSelect), new NameValueCollection
            {
                {"textBoxID", textBoxId}
            }));
        }
        ...
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                spContents.SelectCommand = MaiDarDataProvider.UserDao.GetSelectCommand(true);
            }
            else
            {
                spContents.SelectCommand = MaiDarDataProvider.UserDao.GetSelectCommand(Request.QueryString["keyword"], TranslateUtils.ToInt(Request.QueryString["createDate"]), TranslateUtils.ToInt(Request.QueryString["lastActivityDate"]), true, 0, string.Empty);
            }

            rptContents.ItemDataBound += rptContents_ItemDataBound;
            spContents.SortField = MaiDarDataProvider.UserDao.GetSortFieldName();
...
");
        ...
        private string PageUrl
        {
            get
            {
                ...
                {
                    _pageUrl = PageUtils.GetSettingsUrl(nameof(ModalUserSelect), new NameValueCollection
                    {
