...
namespace MaiDarServer.BackgroundPages.Settings
{
    public class PageUserAdd : BasePage
    {
        ...
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            if (_userId > 0)
            {
                var userInfo = MaiDarDataProvider.UserDao.GetUserInfo(_userId);
                if (userInfo != null)
                {
        ...
        public override void Submit_OnClick(object sender, EventArgs e)
        {
            if (Page.IsPostBack && Page.IsValid)
            {
                if (_userId == 0)
                {
                    ...
                    string errorMessage;
                    var isCreated = MaiDarDataProvider.UserDao.Insert(userInfo, userInfo.Password, string.Empty, out errorMessage);
