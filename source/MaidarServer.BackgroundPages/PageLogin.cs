...
namespace MaiDarServer.BackgroundPages
{
    public class PageLogin : BasePage
	{
		...
		public override void Submit_OnClick(object sender, EventArgs e)
		{
            ...
            string userName;
            string errorMessage;
            if (!MaiDarDataProvider.AdministratorDao.ValidateAccount(account, password, out userName, out errorMessage)) // 检测密码是否正确
            {
                LogUtils.AddAdminLog(userName, "后台管理员登录失败");
                MaiDarDataProvider.AdministratorDao.UpdateLastActivityDateAndCountOfFailedLogin(userName); // 记录最后登录时间、失败次数+1
                LtlMessage.Text = GetMessageHtml(errorMessage); // 把错误信息显示在页面上
                return;
            }

            MaiDarDataProvider.AdministratorDao.UpdateLastActivityDateAndCountOfLogin(userName); // 记录最后登录时间、失败次数清零
