...
namespace MaiDarServer.BackgroundPages.Cms
{
    public class ModalContentTaxis : BasePageCms
    {
        ...
        public override void Submit_OnClick(object sender, EventArgs e)
        {
            ...
            {
                var isTop = TranslateUtils.ToBool(MaiDarDataProvider.ContentDao.GetValue(_tableName, contentId, ContentAttribute.IsTop));
                for (var i = 1; i <= taxisNum; i++)
                {
                    if (isUp)
                    {
                        if (MaiDarDataProvider.ContentDao.UpdateTaxisToUp(_tableName, _nodeId, contentId, isTop) == false)
                        {
                            break;
                        }
                    }
                    ...
                    {
                        if (MaiDarDataProvider.ContentDao.UpdateTaxisToDown(_tableName, _nodeId, contentId, isTop) == false)
                        {
                            break;
                        }
                    }
