...
using MaiDarServer.BackgroundPages.Ajax;
...
using MaiDarServer.CMS.Core.Office;
        ...
        private int SaveContentInfo(bool isPreview, out string errorMessage)
        {
            ...
            if (contentId == 0)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            //添加审核记录
                            MaiDarDataProvider.ContentDao.UpdateIsChecked(_tableName, PublishmentSystemId, contentInfo.NodeId, new List<int> { savedContentId }, 0, true, Body.AdminName, contentInfo.IsChecked, contentInfo.CheckedLevel, "");
                        }
            ...
            else
            {
                ...
                try
                {
                    ...
                    var tableList = MaiDarDataProvider.TableCollectionDao.GetAuxiliaryTableListCreatedInDbByAuxiliaryTableType(EAuxiliaryTableType.BackgroundContent);
                    foreach (var table in tableList)
                    {
                        var targetContentIdList = MaiDarDataProvider.ContentDao.GetReferenceIdList(table.TableEnName, sourceContentIdList);
                        foreach (var targetContentId in targetContentIdList)
                        {
                            ...
                            MaiDarDataProvider.ContentDao.Update(table.TableEnName, contentInfo);
