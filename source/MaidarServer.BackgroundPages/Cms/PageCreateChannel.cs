...
namespace MaiDarServer.BackgroundPages.Cms
{
    public class PageCreateChannel : BasePageCms
    {
        ...
        public void CreateNodeButton_OnClick(object sender, EventArgs e)
        {
            ...
            {
                var lastEditList = MaiDarDataProvider.ContentDao.GetNodeIdListCheckedByLastEditDateHour(tableName, PublishmentSystemId, 720);
            ...
            {
                var lastEditList = MaiDarDataProvider.ContentDao.GetNodeIdListCheckedByLastEditDateHour(tableName, PublishmentSystemId, 24);
                foreach (var nodeId in lastEditList)
                {
                    if (selectedNodeIdArrayList.Contains(nodeId.ToString()))
                    {
            ...
            {
                var lastEditList = MaiDarDataProvider.ContentDao.GetNodeIdListCheckedByLastEditDateHour(tableName, PublishmentSystemId, 2);
                foreach (var nodeId in lastEditList)
                {
                    if (selectedNodeIdArrayList.Contains(nodeId.ToString()))
                    {
