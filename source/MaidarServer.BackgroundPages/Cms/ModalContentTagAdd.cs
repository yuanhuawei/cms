		...
		public void Page_Load(object sender, EventArgs e)
        {
			...
			if (!IsPostBack)
			{
                if (!string.IsNullOrEmpty(_tagName))
                {
                    ...
                    var count = MaiDarDataProvider.TagDao.GetTagCount(_tagName, PublishmentSystemId);
			...
			if (!string.IsNullOrEmpty(_tagName))
			{
				try
				{
                    if (!string.Equals(_tagName, tbTags.Text))
                    {
                        ...
                        var contentIDArrayList = MaiDarDataProvider.TagDao.GetContentIdListByTag(_tagName, PublishmentSystemId);
                        if (contentIDArrayList.Count > 0)
                        {
                            foreach (int contentID in contentIDArrayList)
                            {
                                ...
                                {
                                    var tagInfo = MaiDarDataProvider.TagDao.GetTagInfo(PublishmentSystemId, _tagName);
                                    if (tagInfo != null)
                                    {
                                        ...
                                        MaiDarDataProvider.TagDao.Update(tagInfo);
                                    }
                                }

                                TagUtils.AddTags(tagCollection, PublishmentSystemId, contentID);

                                var contentTags = MaiDarDataProvider.ContentDao.GetValue(PublishmentSystemInfo.AuxiliaryTableForContent, contentID, ContentAttribute.Tags);
                                ...
                                MaiDarDataProvider.ContentDao.SetValue(PublishmentSystemInfo.AuxiliaryTableForContent, contentID, ContentAttribute.Tags, TranslateUtils.ObjectCollectionToString(contentTagArrayList));
                            }
                        }
                        else
                        {
                            MaiDarDataProvider.TagDao.DeleteTag(_tagName, PublishmentSystemId);
                        }
                    }
