    ...
    public class PageTableStyleContent : BasePageCms
    {
        ...
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                InfoMessage(
                    $"在此编辑内容模型字段,子栏目默认继承父栏目字段设置; 辅助表:{MaiDarDataProvider.TableCollectionDao.GetTableCnName(_tableName)}({_tableName})");
        ...
        private void SetTaxis()
        {
            ...
            var styleInfo = MaiDarDataProvider.TableStyleDao.GetTableStyleInfo(tableStyleId);
            if (styleInfo != null && styleInfo.RelatedIdentity == _nodeInfo.NodeId)
            {
                ...
                {
                    case "UP":
                        MaiDarDataProvider.TableStyleDao.TaxisDown(tableStyleId);
                        break;
                    case "DOWN":
                        MaiDarDataProvider.TableStyleDao.TaxisUp(tableStyleId);
                        break;
                }
                SuccessMessage("排序成功！");
            }
        ...
        void dgContents_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                {
                    var tableMetadataId = MaiDarDataProvider.TableMetadataDao.GetTableMetadataId(styleInfo.TableName, styleInfo.AttributeName);
