...
namespace MaiDarServer.BackgroundPages.Plugins
{
	public class PageGatherDatabaseRuleAdd : BasePageCms
    {
        ...
        {
            return PageUtils.GetPluginsUrl(nameof(PageGatherDatabaseRuleAdd), new NameValueCollection
            {
        ...
        public void DatabaseName_Changed(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (isSuccess)
                {
                    var dictionary = MaiDarDataProvider.DatabaseDao.GetTablesAndViewsDictionary(GetDatabaseConnectionString(), DatabaseName.SelectedValue);
                    RelatedTableName.Items.Clear();
        ...
        public void RelatedTable_Changed(object sender, EventArgs e)
        {
            ...
            {
                var tableColumnInfoList = MaiDarDataProvider.DatabaseDao.GetTableColumnInfoList(GetDatabaseConnectionString(), DatabaseName.SelectedValue, RelatedTableName.SelectedValue);
                RelatedIdentity.Items.Clear();
                RelatedOrderBy.Items.Clear();
                ...
                {
                    item = new ListItem(
                        $"{tableColumnInfo.ColumnName}({DataTypeUtils.GetValue(tableColumnInfo.DataType)} {tableColumnInfo.Length})", tableColumnInfo.ColumnName);
        ...
        private bool Validate_GatherDatabaseLogin(out string errorMessage)
        {
            ...
            var databaseNameList = MaiDarDataProvider.DatabaseDao.GetDatabaseNameList(GetConnectionString());
        ...
        private bool Validate_GatherRelatedTable(out string errorMessage)
        {
            ...
            if (_isEdit)
            {
                ...
                if (tableMatchID != 0)
                {
                    var tableMatchInfo = MaiDarDataProvider.TableMatchDao.GetTableMatchInfo(tableMatchID);
			...
			try
			{
				...
				if (_isEdit)
				{
                    ...
                    else
                    {
                        ...
                        var tableMatchInfo =
                            MaiDarDataProvider.TableMatchDao.GetTableMatchInfo(gatherDatabaseRuleInfo.TableMatchId);
                        if (tableMatchInfo == null)
                        {
                            tableMatchInfo =
                            ...
                            gatherDatabaseRuleInfo.TableMatchId =
                                MaiDarDataProvider.TableMatchDao.Insert(tableMatchInfo);
                        }
                        else
                        {
                            ...
                            MaiDarDataProvider.TableMatchDao.Update(tableMatchInfo);
                        }
                ...
                if (isNeedAdd)
                {
                    ...
                    gatherDatabaseRuleInfo.TableMatchId = MaiDarDataProvider.TableMatchDao.Insert(tableMatchInfo);
        ...
        private void SetColumns(NameValueCollection columnsMap)
        {
            ...
            {
                string text =
                    $"{tableColumnInfo.ColumnName}({DataTypeUtils.GetValue(tableColumnInfo.DataType)} {tableColumnInfo.Length})";
                ...
                if (!string.IsNullOrEmpty(columnToMatch))
                {
                    ...
                    if (tableMetadataInfoToMatch != null)
                    {
                        ...
                        text += " -> " +
                                $"{tableMetadataInfoToMatch.AttributeName}({DataTypeUtils.GetValue(tableMetadataInfoToMatch.DataType)} {tableMetadataInfoToMatch.DataLength})";
                        value += "&" + columnToMatch;
                    }
                }
            ...
            foreach (var tableMetadataInfo in tableMetadataInfoList)
            {
                ...
                {
                    string text =
                        $"{tableMetadataInfo.AttributeName}({DataTypeUtils.GetValue(tableMetadataInfo.DataType)} {tableMetadataInfo.DataLength})";
