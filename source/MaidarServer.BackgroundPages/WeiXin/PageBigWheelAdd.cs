...
{
    public class PageBigWheelAdd : BasePageCms
    {
        ...
        public Literal LtlContentAwardImageUrl;
        ...
        public HtmlInputHidden ContentAwardImageUrl;
        ...
        {
            return PageUtils.GetWeiXinUrl(nameof(PageBigWheelAdd), new NameValueCollection
            {
        ...
        public override void Submit_OnClick(object sender, EventArgs e)
        {
            if (Page.IsPostBack && Page.IsValid)
            {
                ...
                else if (selectedStep == 3)
                {
                    ...
                    {
                        FailMessage("大转盘保存失败，每人每天最多允许抽奖次数必须小于每人最多抽奖次数");
                        isItemReady = false;
                    }
                    ...
                    if (isItemReady)
                    {
                        ...
                        if (itemCount < 1)
                        {
                            FailMessage("大转盘保存失败，至少需要设置一个奖项");
                            isItemReady = false;
                        }
                        else
                        {
                            ...
                            {
                                FailMessage("大转盘保存失败，获奖概率之和必须在1%到100%之间");
                                isItemReady = false;
                            }
                ...
                else if (selectedStep == 5)
                {
                    ...
                    lotteryInfo.LotteryType = ELotteryTypeUtils.GetValue(ELotteryType.BigWheel);

                    lotteryInfo.KeywordId = DataProviderWx.KeywordDao.GetKeywordId(PublishmentSystemId, _lotteryId > 0, TbKeywords.Text, EKeywordType.BigWheel, lotteryInfo.KeywordId);
                    ...
                    lotteryInfo.ContentAwardImageUrl = ContentAwardImageUrl.Value;
                    ...
                    try
                    {
                        if (_lotteryId > 0)
                        {
                            ...
                            LogUtils.AddAdminLog("修改大转盘",
                                $"大转盘:{TbTitle.Text}");
                            SuccessMessage("修改大转盘成功！");
                        }
                        else
                        {
                            ...
                            LogUtils.AddAdminLog("添加大转盘",
                                $"大转盘:{TbTitle.Text}");
                            SuccessMessage("添加大转盘成功！");
                        }

                        var redirectUrl = PageLottery.GetRedirectUrl(PublishmentSystemId, ELotteryType.BigWheel);
                        AddWaitAndRedirectScript(redirectUrl);
                    }
                    catch (Exception ex)
                    {
                        FailMessage(ex, "大转盘设置失败！");
                    }
                }
            }
        }
