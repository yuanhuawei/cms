...
{
    public class PageFlapAdd : BasePageCms
    {
        ...
        {
            return PageUtils.GetWeiXinUrl(nameof(PageFlapAdd), new NameValueCollection
            {
		...
		public void Page_Load(object sender, EventArgs e)
        {
			...
			if (!IsPostBack)
            {
                var pageTitle = _lotteryId > 0 ? "编辑大翻牌" : "添加大翻牌";
                BreadCrumb(AppManager.WeiXin.LeftMenu.Function.IdFlap, pageTitle, AppManager.WeiXin.Permission.WebSite.Flap);
                LtlPageTitle.Text = pageTitle;

                LtlImageUrl.Text =
                    $@"<img id=""preview_imageUrl"" src=""{LotteryManager.GetImageUrl(PublishmentSystemInfo,
                        ELotteryType.Flap, string.Empty)}"" width=""370"" align=""middle"" />";
                LtlContentImageUrl.Text =
                    $@"<img id=""preview_contentImageUrl"" src=""{LotteryManager.GetContentImageUrl(
                        PublishmentSystemInfo, ELotteryType.Flap, string.Empty)}"" width=""370"" align=""middle"" />";
...
itemController.itemCount = {awardInfoList.Count};itemController.items = [{itemBuilder}];";
                ...
                BtnReturn.Attributes.Add("onclick",
                    $@"location.href=""{PageLottery.GetRedirectUrl(PublishmentSystemId, ELotteryType.Flap)}"";return false");
			}
		}
		...
		public override void Submit_OnClick(object sender, EventArgs e)
		{
			if (Page.IsPostBack && Page.IsValid)
			{
                ...
                else if (selectedStep == 3)
                {
                    ...
                    {
                        FailMessage("大翻牌保存失败，每人每天最多允许抽奖次数必须小于每人最多抽奖次数");
                        isItemReady = false;
                    }
                    ...
                    if (isItemReady)
                    {
                        ...
                        if (itemCount < 1)
                        {
                            FailMessage("大翻牌保存失败，至少需要设置一个奖项");
                            isItemReady = false;
                        }
                        else
                        {
                            ...
                            {
                                FailMessage("大翻牌保存失败，获奖概率之和必须在1%到100%之间");
                                isItemReady = false;
                            }
                ...
                else if (selectedStep == 5)
                {
                    ...
                    lotteryInfo.KeywordId = DataProviderWx.KeywordDao.GetKeywordId(PublishmentSystemId, _lotteryId > 0, TbKeywords.Text, EKeywordType.Flap, lotteryInfo.KeywordId);
                    ...
                    try
                    {
                        if (_lotteryId > 0)
                        {
                            ...
                            LogUtils.AddAdminLog(Body.AdminName, "修改大翻牌", $"大翻牌:{TbTitle.Text}");
                            SuccessMessage("修改大翻牌成功！");
                        }
                        else
                        {
                            ...
                            LogUtils.AddAdminLog(Body.AdminName, "添加大翻牌", $"大翻牌:{TbTitle.Text}");
                            SuccessMessage("添加大翻牌成功！");
                        }

                        var redirectUrl = PageLottery.GetRedirectUrl(PublishmentSystemId, ELotteryType.Flap);
                        AddWaitAndRedirectScript(redirectUrl);
                    }
                    catch (Exception ex)
                    {
                        FailMessage(ex, "大翻牌设置失败！");
                    }
                }
			}
		}
