...
{
    public class PageCollect : BasePageCms
    {
        ...
        {
            return PageUtils.GetWeiXinUrl(nameof(PageCollect), new NameValueCollection
            {
        ...
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                BtnDelete.Attributes.Add("onclick", PageUtils.GetRedirectStringWithCheckBoxValueAndAlert(urlDelete, "IDCollection", "IDCollection", "请选择需要删除的征集活动", "此操作将删除所选征集活动，确认吗？"));
            }
        }
        ...
        void rptContents_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                var urlCollect = PageCollectItem.GetRedirectUrl(PublishmentSystemId, collectInfo.Id, GetRedirectUrl(PublishmentSystemId));
                ltlCollectUrl.Text = $@"<a href=""{urlCollect}"">参赛记录</a>";

                var urlLog = PageCollectLog.GetRedirectUrl(PublishmentSystemId, collectInfo.Id, GetRedirectUrl(PublishmentSystemId));
                ...
                var urlEdit = PageCollectAdd.GetRedirectUrl(PublishmentSystemId, collectInfo.Id);
