...
{
    public class PageCardExchangeLog : BasePageCms
    {
        ...
        {
            return PageUtils.GetWeiXinUrl(nameof(PageCardExchangeLog), new NameValueCollection
            {
        ...
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["Delete"])) 
            {
                ...
                if (list.Count > 0)
                {
                    try
                    {
                        ...
                        SuccessMessage("积分兑换记录删除成功！");
                    }
                    catch (Exception ex)
                    {
                        FailMessage(ex, "积分兑换记录删除失败！");
                    }
                }
            }
            ...
            SpContents.SelectCommand = DataProviderWx.CardCashLogDao.GetSelectString(PublishmentSystemId, ECashType.Exchange, TranslateUtils.ToInt(Request.QueryString["cardID"]), Request.QueryString["cardSN"], Request.QueryString["userName"], Request.QueryString["mobile"]);
            ...
            if (!IsPostBack)
            {
                ...
                BreadCrumb(AppManager.WeiXin.LeftMenu.Function.IdCard, "会员积分兑换管理", AppManager.WeiXin.Permission.WebSite.Card);
                ...
                var urlDelete = PageUtils.AddQueryString(PageCardConsumeLog.GetRedirectUrl(PublishmentSystemId, 0, TbCardSn.Text, TbUserName.Text, TbMobile.Text), "Delete", "True");
                BtnDelete.Attributes.Add("onclick", PageUtils.GetRedirectStringWithCheckBoxValueAndAlert(urlDelete, "IDCollection", "IDCollection", "请选择需要删除的兑换记录", "此操作将删除所选兑换记录，确认吗？"));
        ...
        void rptContents_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                var userInfo = MaiDarDataProvider.UserDao.GetUserInfoByUserName(cardCashLogInfo.UserName);
