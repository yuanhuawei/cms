...
{
    public class PageYaoYaoAdd : BasePageCms
    {
        ...
        {
            return PageUtils.GetWeiXinUrl(nameof(PageYaoYaoAdd), new NameValueCollection
            {
		...
		public void Page_Load(object sender, EventArgs e)
        {
			...
			if (!IsPostBack)
            {
                var pageTitle = _lotteryId > 0 ? "编辑摇摇乐" : "添加摇摇乐";
                BreadCrumb(AppManager.WeiXin.LeftMenu.Function.IdYaoYao, pageTitle, AppManager.WeiXin.Permission.WebSite.YaoYao);
...
itemController.itemCount = {awardInfoList.Count};itemController.items = [{itemBuilder}];";
                ...
                BtnReturn.Attributes.Add("onclick",
                    $@"location.href=""{PageLottery.GetRedirectUrl(PublishmentSystemId, ELotteryType.YaoYao)}"";return false");
			}
		}
                ...
                else if (selectedStep == 5)
                {
                    ...
                    try
                    {
                        if (_lotteryId > 0)
                        {
                            ...
                            LogUtils.AddAdminLog(Body.AdminName, "修改摇摇乐",
                                $"摇摇乐:{TbTitle.Text}");
                        ...
                        else
                        {
                            ...
                            LogUtils.AddAdminLog(Body.AdminName, "添加摇摇乐",
                                $"摇摇乐:{TbTitle.Text}");
