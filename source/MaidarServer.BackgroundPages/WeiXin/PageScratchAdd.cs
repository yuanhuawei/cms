...
{
	public class PageScratchAdd : BasePageCms
    {
        ...
        {
            return PageUtils.GetWeiXinUrl(nameof(PageScratchAdd), new NameValueCollection
            {
			...
			if (!IsPostBack)
            {
                ...
                LtlImageUrl.Text =
                    $@"<img id=""preview_imageUrl"" src=""{LotteryManager.GetImageUrl(PublishmentSystemInfo,
                        ELotteryType.Scratch, string.Empty)}"" width=""370"" align=""middle"" />";
                LtlContentImageUrl.Text =
                    $@"<img id=""preview_contentImageUrl"" src=""{LotteryManager.GetContentImageUrl(
                        PublishmentSystemInfo, ELotteryType.Scratch, string.Empty)}"" width=""370"" align=""middle"" />";
...
itemController.itemCount = {awardInfoList.Count};itemController.items = [{itemBuilder}];";
                ...
                BtnReturn.Attributes.Add("onclick",
                    $@"location.href=""{PageLottery.GetRedirectUrl(PublishmentSystemId, ELotteryType.Scratch)}"";return false");
			}
		}
		...
		public override void Submit_OnClick(object sender, EventArgs e)
		{
			if (Page.IsPostBack && Page.IsValid)
			{
                ...
                else if (selectedStep == 3)
                {
                    ...
                    {
                        FailMessage("刮刮卡保存失败，每人每天最多允许抽奖次数必须小于每人最多抽奖次数");
                        isItemReady = false;
                    }
                    ...
                    if (isItemReady)
                    {
                        ...
                        if (itemCount < 1)
                        {
                            FailMessage("刮刮卡保存失败，至少需要设置一个奖项");
                            isItemReady = false;
                        }
                        else
                        {
                            ...
                            {
                                FailMessage("刮刮卡保存失败，获奖概率之和必须在1%到100%之间");
                                isItemReady = false;
                            }
                ...
                else if (selectedStep == 5)
                {
                    ...
                    lotteryInfo.LotteryType = ELotteryTypeUtils.GetValue(ELotteryType.Scratch);

                    lotteryInfo.KeywordId = DataProviderWx.KeywordDao.GetKeywordId(PublishmentSystemId, _lotteryId > 0, TbKeywords.Text, EKeywordType.Scratch, lotteryInfo.KeywordId);
                    ...
                    lotteryInfo.ContentAwardImageUrl = string.Empty;
                    ...
                    try
                    {
                        ...
                        var redirectUrl = PageLottery.GetRedirectUrl(PublishmentSystemId, ELotteryType.Scratch);
                        AddWaitAndRedirectScript(redirectUrl);
                    }
                    catch (Exception ex)
                    {
                        FailMessage(ex, "刮刮卡设置失败！");
                    }
                }
			}
		}
