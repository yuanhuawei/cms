...
namespace MaiDarServer.BackgroundPages.Ajax
{
    public class AjaxCmsService : Page
    {
        ...
        public string GetTitles(int publishmentSystemId, int nodeId, string title)
        {
            ...
            var titleList = MaiDarDataProvider.ContentDao.GetValueListByStartString(tableName, nodeId, ContentAttribute.Title, title, 10);
        ...
        public string GetTags(int publishmentSystemId, string tag)
        {
            ...
            var tagList = MaiDarDataProvider.TagDao.GetTagListByStartString(publishmentSystemId, tag, 10);
            if (tagList.Count > 0)
            {
