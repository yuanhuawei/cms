    ...
    public class Database
    {
        ...
        public static int GetPageTotalCount(string sqlString)
        {
            ...
            lock (LockObject)
            {
                ...
                if (retval == -1)
                {
                    retval = MaiDarDataProvider.DatabaseDao.GetPageTotalCount(sqlString);
        ...
        public static string GetStlPageSqlString(string sqlString, string orderByString, int totalNum, int pageNum, int currentPageIndex)
        {
            ...
            lock (LockObject)
            {
                ...
                if (retval == null)
                {
                    retval = MaiDarDataProvider.DatabaseDao.GetStlPageSqlString(sqlString, orderByString, totalNum, pageNum,
                    currentPageIndex);
                    StlCacheUtils.SetCache(cacheKey, retval);
                }
            }
        ...
        public static string GetString(string connectionString, string queryString)
        {
            ...
            lock (LockObject)
            {
                ...
                if (retval == null)
                {
                    retval = MaiDarDataProvider.DatabaseDao.GetString(connectionString, queryString);
                    StlCacheUtils.SetCache(cacheKey, retval);
                }
            }
