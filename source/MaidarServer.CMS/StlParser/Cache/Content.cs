    ...
    public class Content
    {
        ...
        public static string GetValue(string tableName, int contentId, string type)
        {
            ...
            lock (LockObject)
            {
                ...
                if (retval == null)
                {
                    retval = MaiDarDataProvider.ContentDao.GetValue(tableName, contentId, type);
                    StlCacheUtils.SetCache(cacheKey, retval);
                }                
            }
        ...
        public static int GetSequence(string tableName, int nodeId, int contentId)
        {
            ...
            lock (LockObject)
            {
                ...
                if (retval == -1)
                {
                    retval = MaiDarDataProvider.ContentDao.GetSequence(tableName, nodeId, contentId);
                    StlCacheUtils.SetCache(cacheKey, retval);
                }
            }
        ...
        public static DateTime GetAddDate(string tableName, int contentId)
        {
            ...
            lock (LockObject)
            {
                ...
                {
                    retval = MaiDarDataProvider.ContentDao.GetAddDate(tableName, contentId);
                    StlCacheUtils.SetCache(cacheKey, retval);
                }
            }
        ...
        public static DateTime GetLastEditDate(string tableName, int contentId)
        {
            ...
            lock (LockObject)
            {
                ...
                if (retval == DateTime.MinValue)
                {
                    retval = MaiDarDataProvider.ContentDao.GetLastEditDate(tableName, contentId);
                    StlCacheUtils.SetCache(cacheKey, retval);
                }
            }
        ...
        public static int GetContentId(string tableName, int nodeId, int taxis, bool isNextContent)
        {
            ...
            lock (LockObject)
            {
                ...
                if (retval == -1)
                {
                    retval = MaiDarDataProvider.ContentDao.GetContentId(tableName, nodeId, taxis, isNextContent);
                    StlCacheUtils.SetCache(cacheKey, retval);
                }
            }
        ...
        public static int GetContentId(string tableName, int nodeId, string orderByString)
        {
            ...
            lock (LockObject)
            {
                ...
                if (retval == -1)
                {
                    retval = MaiDarDataProvider.ContentDao.GetContentId(tableName, nodeId, orderByString);
                    StlCacheUtils.SetCache(cacheKey, retval);
                }
            }
        ...
        public static int GetNodeId(string tableName, int contentId)
        {
            ...
            lock (LockObject)
            {
                ...
                if (retval == -1)
                {
                    retval = MaiDarDataProvider.ContentDao.GetNodeId(tableName, contentId);
                    StlCacheUtils.SetCache(cacheKey, retval);
                }
            }
        ...
        public static string GetStlWhereString(int publishmentSystemId, string group, string groupNot, string tags, bool isTopExists, bool isTop, string where)
        {
            ...
            lock (LockObject)
            {
                ...
                if (retval == null)
                {
                    retval = MaiDarDataProvider.ContentDao.GetStlWhereString(publishmentSystemId, group, groupNot, tags,
