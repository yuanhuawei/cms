...
namespace MaiDarServer.CMS.StlParser.StlEntity
{
                ...
                try
                {
                    ...
                    if (StringUtils.EqualsIgnoreCase(ContentAttribute.Id, attributeName))//内容ID
                    {
                        ...
                        else
                        {
                            ...
                            //parsedContent = MaiDarDataProvider.ContentDao.GetValue(tableName, contextInfo.ContentId, ContentAttribute.Id);
                    ...
                    else if (StringUtils.EqualsIgnoreCase(Title, attributeName))//内容标题
                    {
                        ...
                        {
                            var tableName = NodeManager.GetTableName(pageInfo.PublishmentSystemInfo, NodeManager.GetNodeInfo(pageInfo.PublishmentSystemId, contextInfo.ChannelId));
                            //parsedContent = MaiDarDataProvider.ContentDao.GetValue(tableName, contextInfo.ContentId, ContentAttribute.Title);
                    ...
                    else if (StringUtils.EqualsIgnoreCase(ImageUrl, attributeName))//内容图片地址
                    {
                        ...
                        {
                            var tableName = NodeManager.GetTableName(pageInfo.PublishmentSystemInfo, NodeManager.GetNodeInfo(pageInfo.PublishmentSystemId, contextInfo.ChannelId));
                            //parsedContent = MaiDarDataProvider.ContentDao.GetValue(tableName, contextInfo.ContentId, BackgroundContentAttribute.ImageUrl);
                    ...
                    else if (StringUtils.EqualsIgnoreCase(VideoUrl, attributeName))//内容视频地址
                    {
                        ...
                        {
                            var tableName = NodeManager.GetTableName(pageInfo.PublishmentSystemInfo, NodeManager.GetNodeInfo(pageInfo.PublishmentSystemId, contextInfo.ChannelId));
                            //parsedContent = MaiDarDataProvider.ContentDao.GetValue(tableName, contextInfo.ContentId, BackgroundContentAttribute.VideoUrl);
                    ...
                    else if (StringUtils.EqualsIgnoreCase(FileUrl, attributeName))//内容附件地址
                    {
                        ...
                        {
                            var tableName = NodeManager.GetTableName(pageInfo.PublishmentSystemInfo, NodeManager.GetNodeInfo(pageInfo.PublishmentSystemId, contextInfo.ChannelId));
                            //parsedContent = MaiDarDataProvider.ContentDao.GetValue(tableName, contextInfo.ContentId, BackgroundContentAttribute.FileUrl);
                    ...
                    else if (StringUtils.EqualsIgnoreCase(DownloadUrl, attributeName))//内容附件地址(可统计下载量)
                    {
                        ...
                        if (!string.IsNullOrEmpty(parsedContent))
                        {
                            parsedContent = ActionsDownload.GetUrl(pageInfo.ApiUrl, pageInfo.PublishmentSystemId, contextInfo.ChannelId, contextInfo.ContentId, parsedContent);
                        }
                    }
                    else if (StringUtils.EqualsIgnoreCase(AddDate, attributeName))//内容添加日期
                    {
                        ...
                        else
                        {
                            ...
                            //parsedContent = DateUtils.Format(MaiDarDataProvider.ContentDao.GetAddDate(tableName, contextInfo.ContentId), string.Empty);
                    ...
                    else if (StringUtils.EqualsIgnoreCase(LastEditDate, attributeName))//替换最后修改日期
                    {
                        ...
                        {
                            var tableName = NodeManager.GetTableName(pageInfo.PublishmentSystemInfo, contextInfo.ChannelId);
                            //parsedContent = DateUtils.Format(MaiDarDataProvider.ContentDao.GetLastEditDate(tableName, contextInfo.ContentId), string.Empty);
                    ...
                    else if (StringUtils.EqualsIgnoreCase(Content, attributeName))//内容正文
                    {
                        ...
                        {
                            var tableName = NodeManager.GetTableName(pageInfo.PublishmentSystemInfo, NodeManager.GetNodeInfo(pageInfo.PublishmentSystemId, contextInfo.ChannelId));
                            //parsedContent = MaiDarDataProvider.ContentDao.GetValue(tableName, contextInfo.ContentId, BackgroundContentAttribute.Content);
                    ...
                    else if (StringUtils.EqualsIgnoreCase(Group, attributeName))//内容组别
                    {
                        ...
                        {
                            var tableName = NodeManager.GetTableName(pageInfo.PublishmentSystemInfo, NodeManager.GetNodeInfo(pageInfo.PublishmentSystemId, contextInfo.ChannelId));
                            //parsedContent = MaiDarDataProvider.ContentDao.GetValue(tableName, contextInfo.ContentId, ContentAttribute.ContentGroupNameCollection);
                    ...
                    else if (StringUtils.EqualsIgnoreCase(Tags, attributeName))//标签
                    {
                        ...
                        {
                            var tableName = NodeManager.GetTableName(pageInfo.PublishmentSystemInfo, NodeManager.GetNodeInfo(pageInfo.PublishmentSystemId, contextInfo.ChannelId));
                            //parsedContent = MaiDarDataProvider.ContentDao.GetValue(tableName, contextInfo.ContentId, ContentAttribute.Tags);
                    ...
                    else if (StringUtils.EqualsIgnoreCase(AddUserName, attributeName))
                    {
                        ...
                        {
                            var tableName = NodeManager.GetTableName(pageInfo.PublishmentSystemInfo, NodeManager.GetNodeInfo(pageInfo.PublishmentSystemId, contextInfo.ChannelId));
                            //addUserName = MaiDarDataProvider.ContentDao.GetValue(tableName, contextInfo.ContentId, ContentAttribute.AddUserName);
                        ...
                        {
                            //var displayName = MaiDarDataProvider.AdministratorDao.GetDisplayName(addUserName);
                    ...
                    else
                    {
                        ...
                        {
                            var tableName = NodeManager.GetTableName(pageInfo.PublishmentSystemInfo, contextInfo.ChannelId);
                            //contentNodeId = MaiDarDataProvider.ContentDao.GetNodeId(tableName, contextInfo.ContentId);
                            ...
                            //parsedContent = MaiDarDataProvider.ContentDao.GetValue(tableName, contextInfo.ContentId, attributeName);
