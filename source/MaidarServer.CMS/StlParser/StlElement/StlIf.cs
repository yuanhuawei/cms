        ...
        private static string ParseImpl(PageInfo pageInfo, ContextInfo contextInfo, string testType, string testOperate, string testValue)
        {
            ...
            else if (StringUtils.EqualsIgnoreCase(testType, TypeGroupContent))
            {
                if (contextInfo.ContextType == EContextType.Content)
                {
                    ...
                    //var groupContents = TranslateUtils.StringCollectionToStringList(MaiDarDataProvider.ContentDao.GetValue(tableName, contextInfo.ContentId, ContentAttribute.ContentGroupNameCollection));
...
");
        ...
        private static string GetValueFromContent(PageInfo pageInfo, ContextInfo contextInfo, string testTypeStr)
        {
            ...
            {
                var tableName = NodeManager.GetTableName(pageInfo.PublishmentSystemInfo, contextInfo.ChannelId);
                //theValue = MaiDarDataProvider.ContentDao.GetValue(tableName, contextInfo.ContentId, testTypeStr);
        ...
        private static DateTime GetAddDateByContext(PageInfo pageInfo, ContextInfo contextInfo)
        {
            ...
            if (contextInfo.ContextType == EContextType.Content)
            {
                ...
                {
                    var tableName = NodeManager.GetTableName(pageInfo.PublishmentSystemInfo, contextInfo.ChannelId);
                    //addDate = MaiDarDataProvider.ContentDao.GetAddDate(tableName, contextInfo.ContentId);
        ...
        private static DateTime GetLastEditDateByContext(PageInfo pageInfo, ContextInfo contextInfo)
        {
            ...
            if (contextInfo.ContextType == EContextType.Content)
            {
                ...
                {
                    var tableName = NodeManager.GetTableName(pageInfo.PublishmentSystemInfo, contextInfo.ChannelId);
                    //lastEditDate = MaiDarDataProvider.ContentDao.GetLastEditDate(tableName, contextInfo.ContentId);
