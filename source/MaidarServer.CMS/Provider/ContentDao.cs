...
namespace MaiDarServer.CMS.Provider
{
    public class ContentDao : DataProviderBase
    {
        public int GetTaxisToInsert(string tableName, int nodeId, bool isTop)
        {
            ...
            if (isTop)
            {
                taxis = MaiDarDataProvider.ContentDao.GetMaxTaxis(tableName, nodeId, true) + 1;
            }
            else
            {
                taxis = MaiDarDataProvider.ContentDao.GetMaxTaxis(tableName, nodeId, false) + 1;
            }
        ...
        public int Insert(string tableName, PublishmentSystemInfo publishmentSystemInfo, IContentInfo contentInfo, bool isUpdateContentNum, int taxis)
        {
            ...
            if (!string.IsNullOrEmpty(tableName))
            {
                ...
                contentId = MaiDarDataProvider.ContentDao.Insert(tableName, contentInfo);
        ...
        public void Update(string tableName, PublishmentSystemInfo publishmentSystemInfo, IContentInfo contentInfo)
        {
            ...
            MaiDarDataProvider.ContentDao.Update(tableName, contentInfo);
        ...
        public ContentInfo GetContentInfoNotTrash(ETableStyle tableStyle, string tableName, int contentId)
        {
            ...
            if (contentId > 0)
            {
                if (!string.IsNullOrEmpty(tableName))
                {
                    ...
                    var sqlSelect = MaiDarDataProvider.DatabaseDao.GetSelectSqlString(tableName, SqlUtils.Asterisk, sqlWhere);
                    ...
                    using (var rdr = ExecuteReader(sqlSelect))
                    {
                        if (rdr.Read())
                        {
                            ...
                            MaiDarDataProvider.DatabaseDao.ReadResultsToExtendedAttributes(rdr, info);
                        }
                        rdr.Close();
                    }
                }
        ...
        public int GetCountOfContentAdd(string tableName, int publishmentSystemId, int nodeId, EScopeType scope, DateTime begin, DateTime end, string userName)
        {
            ...
            return MaiDarDataProvider.ContentDao.GetCountOfContentAdd(tableName, publishmentSystemId, nodeIdList, begin, end, userName);
        }
        ...
        {
            var nodeIdList = DataProvider.NodeDao.GetNodeIdListByScopeType(nodeId, scope, string.Empty, string.Empty);
            return MaiDarDataProvider.ContentDao.GetCountOfContentUpdate(tableName, publishmentSystemId, nodeIdList, begin, end, userName);
        }
        ...
        public List<int> GetContentIdListChecked(string tableName, int nodeId, int totalNum, string orderByFormatString, string whereString)
        {
            ...
            return MaiDarDataProvider.ContentDao.GetContentIdListChecked(tableName, nodeIdList, totalNum, orderByFormatString, whereString);
        }
        ...
        public void TrashContents(int publishmentSystemId, string tableName, List<int> contentIdList, int nodeId)
        {
            if (!string.IsNullOrEmpty(tableName))
            {
                var referenceIdList = MaiDarDataProvider.ContentDao.GetReferenceIdList(tableName, contentIdList);
                if (referenceIdList.Count > 0)
                {
                    DeleteContents(publishmentSystemId, tableName, referenceIdList);
                }
                var updateNum = MaiDarDataProvider.ContentDao.TrashContents(publishmentSystemId, tableName, contentIdList);
                if (updateNum > 0)
                {
                    new Action(() =>
                    {
        ...
        public void TrashContents(int publishmentSystemId, string tableName, List<int> contentIdArrayList)
        {
            if (!string.IsNullOrEmpty(tableName))
            {
                var referenceIdList = MaiDarDataProvider.ContentDao.GetReferenceIdList(tableName, contentIdArrayList);
                if (referenceIdList.Count > 0)
                {
                    DeleteContents(publishmentSystemId, tableName, referenceIdList);
                }
                var updateNum = MaiDarDataProvider.ContentDao.TrashContents(publishmentSystemId, tableName, contentIdArrayList);
                if (updateNum > 0)
                {
                    new Action(() =>
                    {
        ...
        public void TrashContentsByNodeId(int publishmentSystemId, string tableName, int nodeId)
        {
            if (!string.IsNullOrEmpty(tableName))
            {
                var contentIdList = MaiDarDataProvider.ContentDao.GetContentIdList(tableName, nodeId);
                var referenceIdList = MaiDarDataProvider.ContentDao.GetReferenceIdList(tableName, contentIdList);
                if (referenceIdList.Count > 0)
                {
                    DeleteContents(publishmentSystemId, tableName, referenceIdList);
                }
                var updateNum = MaiDarDataProvider.ContentDao.TrashContentsByNodeId(publishmentSystemId, tableName, nodeId);
                if (updateNum > 0)
                {
                    new Action(() =>
                    {
        ...
        public void DeleteContents(int publishmentSystemId, string tableName, List<int> contentIdList, int nodeId)
        {
            if (!string.IsNullOrEmpty(tableName))
            {
                var deleteNum = MaiDarDataProvider.ContentDao.DeleteContents(publishmentSystemId, tableName, contentIdList);
        ...
        public void DeleteContentsByNodeId(int publishmentSystemId, string tableName, int nodeId)
        {
            if (!string.IsNullOrEmpty(tableName))
            {
                ...
                var deleteNum = MaiDarDataProvider.ContentDao.DeleteContentsByNodeId(publishmentSystemId, tableName, nodeId, contentIdList);
        ...
        {
            var updateNum = MaiDarDataProvider.ContentDao.RestoreContentsByTrash(publishmentSystemId, tableName);
            if (updateNum > 0)
            {
                new Action(() =>
                {
        ...
        public string GetSelectCommend(ETableStyle tableStyle, string tableName, int publishmentSystemId, int nodeId, bool isSystemAdministrator, List<int> owningNodeIdList, string searchType, string keyword, string dateFrom, string dateTo, bool isSearchChildren, ETriState checkedState, bool isNoDup, bool isTrashContent)
        {
            ...
            return MaiDarDataProvider.ContentDao.GetSelectCommendByCondition(tableStyle, tableName, publishmentSystemId, list, searchType, keyword, dateFrom, dateTo, checkedState, isNoDup, isTrashContent);
        }
        ...
        public string GetSelectCommend(ETableStyle tableStyle, string tableName, int publishmentSystemId, int nodeId, bool isSystemAdministrator, List<int> owningNodeIdList, string searchType, string keyword, string dateFrom, string dateTo, bool isSearchChildren, ETriState checkedState, bool isNoDup, bool isTrashContent, bool isWritingOnly, string userNameOnly)
        {
            ...
            return MaiDarDataProvider.ContentDao.GetSelectCommendByCondition(tableStyle, tableName, publishmentSystemId, list, searchType, keyword, dateFrom, dateTo, checkedState, isNoDup, isTrashContent, isWritingOnly, userNameOnly);
        }
        ...
        public string GetWritingSelectCommend(string writingUserName, ETableStyle tableStyle, string tableName, int publishmentSystemId, List<int> nodeIdList, string searchType, string keyword, string dateFrom, string dateTo)
        {
            ...
            return MaiDarDataProvider.DatabaseDao.GetSelectSqlString(tableName, SqlUtils.Asterisk, whereString.ToString());
        }
        ...
        {
            return MaiDarDataProvider.ContentDao.GetStlDataSourceChecked(tableStyle, tableName, nodeIdList, startNum, totalNum, orderByString, whereString, isNoDup, others);
        }
        ...
        public string GetStlSqlStringChecked(List<int> nodeIdList, string tableName, int publishmentSystemId, int nodeId, int startNum, int totalNum, string orderByString, string whereString, EScopeType scopeType, string groupChannel, string groupChannelNot, bool isNoDup)
        {
            ...
            if (isNoDup)
            {
                var sqlString = MaiDarDataProvider.DatabaseDao.GetSelectSqlString(tableName, "MIN(Id)", sqlWhereString + " GROUP BY Title");
                sqlWhereString += $" AND Id IN ({sqlString})";
            }

            if (!string.IsNullOrEmpty(tableName))
            {
                return MaiDarDataProvider.DatabaseDao.GetSelectSqlString(tableName, startNum, totalNum, MaiDarDataProvider.ContentDao.StlColumns, sqlWhereString, orderByString);
            }
            return string.Empty;
        }
        ...
        public string GetStlSqlStringCheckedBySearch(string tableName, int startNum, int totalNum, string orderByString, string whereString, bool isNoDup)
        {
            ...
            if (!string.IsNullOrEmpty(tableName))
            {
                return MaiDarDataProvider.DatabaseDao.GetSelectSqlString(tableName, startNum, totalNum, $"{ContentAttribute.Id}, {ContentAttribute.NodeId}, {ContentAttribute.IsTop}, {ContentAttribute.AddDate}", sqlWhereString, orderByString);
            }
            return string.Empty;
        }
        ...
        public void TidyUp(string tableName, int nodeId, string attributeName, bool isDesc)
        {
            ...
            MaiDarDataProvider.DatabaseDao.ExecuteSql(sqlList);
        }
