...
namespace MaiDarServer.CMS.WeiXin.Provider
{
    public class WebMenuDao : DataProviderBase
    {
        ...
        public int Insert(WebMenuInfo menuInfo)
        {
            ...
            var sqlInsert = MaiDarDataProvider.DatabaseDao.GetInsertSqlString(menuInfo.ToNameValueCollection(), ConnectionString, TableName, out parms);
        ...
        {
            IDataParameter[] parms = null;
            var sqlUpdate = MaiDarDataProvider.DatabaseDao.GetUpdateSqlString(menuInfo.ToNameValueCollection(), ConnectionString, TableName, out parms);
        ...
        public IEnumerable GetDataSource(int publishmentSystemId, int parentId)
        {
            string whereString =
                $"WHERE {WebMenuAttribute.PublishmentSystemId} = {publishmentSystemId} AND {WebMenuAttribute.ParentId} = {parentId}";
            var sqlString = MaiDarDataProvider.DatabaseDao.GetSelectSqlString(TableName, SqlUtils.Asterisk, whereString);
