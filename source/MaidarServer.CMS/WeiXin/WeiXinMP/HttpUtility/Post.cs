...
using MaiDarServer.CMS.WeiXin.WeiXinMP.Exceptions;

namespace MaiDarServer.CMS.WeiXin.WeiXinMP.HttpUtility
{
    public static class Post
    {
            ...
            if (returnText.Contains("errcode"))
            {
                ...
                if (errorResult.errcode != ReturnCode.请求成功)
                {
                    ...
                    throw new ErrorJsonResultException(
                        $"微信Post请求发生错误！错误代码：{(int) errorResult.errcode}，说明：{errorResult.errmsg}",
                        null, errorResult);
                }
            }
        ...
        {
            var returnText = RequestUtility.HttpPost(url, cookieContainer, null, fileDictionary, null, encoding);
        ...
        {
            var returnText = RequestUtility.HttpPost(url, cookieContainer, fileStream, null, null, encoding);
            var result = GetResult<T>(returnText);
            return result;
        }
        ...
        {
            var returnText = RequestUtility.HttpPost(url, cookieContainer, formData, encoding);
            var result = GetResult<T>(returnText);
            return result;
        }
    }
