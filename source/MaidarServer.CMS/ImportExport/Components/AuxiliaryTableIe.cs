...
namespace MaiDarServer.CMS.ImportExport.Components
{
	internal class AuxiliaryTableIe
	{
		...
		{
            var tableInfo = MaiDarDataProvider.TableCollectionDao.GetAuxiliaryTableInfo(tableName);
			if (tableInfo != null)
			{
        ...
        public NameValueCollection ImportAuxiliaryTables(int publishmentSystemId, bool isUserTables)
		{
            ...
            foreach (var filePath in filePaths)
            {
                ...
                {
                    if (!MaiDarDataProvider.DatabaseDao.IsTableExists(tableNameToInsert))
                    {
                        ...
                        foreach (AtomEntry entry in feed.Entries)
                        {
                            ...
                            MaiDarDataProvider.TableMetadataDao.Insert(metaInfo);
                        }

                        MaiDarDataProvider.TableMetadataDao.CreateAuxiliaryTable(tableNameToInsert);
                    }
                }
