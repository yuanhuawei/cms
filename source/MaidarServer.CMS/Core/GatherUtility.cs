    ...
    public class GatherUtility
    {
            ...
            try
            {
                ...
                if (!isSameTitleAllowed)
                {
                    var contentTitles = contentTitleHashtable[channelId] as List<string> ??
                                        MaiDarDataProvider.ContentDao.GetValueList(tableName, channelId, ContentAttribute.Title);
        ...
        public static void GatherDatabase(int publishmentSystemId, string gatherRuleName, StringBuilder resultBuilder, StringBuilder errorBuilder, bool isCache, string userKeyPrefix, string administratorName)
        {
            ...
            try
            {
                ...
                var tableMatchInfo = MaiDarDataProvider.TableMatchDao.GetTableMatchInfo(gatherDatabaseRuleInfo.TableMatchId);
                ...
                {
                    totalCount = MaiDarDataProvider.DatabaseDao.GetIntResult(gatherDatabaseRuleInfo.ConnectionString,
                ...
                var titleList = MaiDarDataProvider.ContentDao.GetValueList(tableName, gatherDatabaseRuleInfo.NodeId, ContentAttribute.Title);

                using (var rdr = MaiDarDataProvider.DatabaseDao.GetDataReader(gatherDatabaseRuleInfo.ConnectionString, sqlString))
                {
                    while (rdr.Read())
                    {
                        try
                        {
                            ...
                            MaiDarDataProvider.DatabaseDao.ReadResultsToNameValueCollection(rdr, collection);
