...
namespace MaiDarServer.CMS.Core.Office
{
    public class ExcelObject
    {
            ...
            string endDate, ETriState checkedState)
        {
            ...
            {
                contentIdList = MaiDarDataProvider.ContentDao.GetContentIdList(tableName, nodeInfo.NodeId, isPeriods,
            ...
            bool isDelete)
        {
            ...
            if (contentId != 0)
            {
                ...
                target = MaiDarDataProvider.ContentDao.GetValue(tableName, contentId, ContentAttribute.Title);
            ...
            foreach (TrackingInfo trackingInfo in trackingInfoArrayList)
            {
                if (contentId == 0)
                {
                    if (trackingInfo.PageContentId != 0)
                    {
                        ...
                        target = MaiDarDataProvider.ContentDao.GetValue(tableName, trackingInfo.PageContentId,
        ...
        public static void CreateExcelFileForUsers(string filePath, ETriState checkedState)
        {
            ...
            var userIdList = MaiDarDataProvider.UserDao.GetUserIdList(checkedState != ETriState.False);
            if (checkedState == ETriState.All)
            {
                userIdList.AddRange(MaiDarDataProvider.UserDao.GetUserIdList(false));
            }

            foreach (var userId in userIdList)
            {
                var userInfo = MaiDarDataProvider.UserDao.GetUserInfo(userId);
