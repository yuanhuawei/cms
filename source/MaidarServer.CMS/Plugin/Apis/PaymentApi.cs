    ...
    public class PaymentApi : IPaymentApi
    {
        ...
        public string ChargeByAlipayPc(string productName, decimal amount, string orderNo, string successUrl)
        {
            ...
            if (config.AlipayPcIsMApi)
            {
                ...
                MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.partner = config.AlipayPcPid;

                // 收款支付宝账号，以2088开头由16位纯数字组成的字符串，一般情况下收款账号就是签约账号
                MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.seller_id = MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.partner;

                // MD5密钥，安全检验码，由数字和字母组成的32位字符串，查看地址：https://b.alipay.com/order/pidAndKey.htm
                MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.key = config.AlipayPcMd5;

                // 服务器异步通知页面路径，需http://格式的完整路径，不能加?id=123这类自定义参数,必须外网可以正常访问
                MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.notify_url = string.Empty;

                // 页面跳转同步通知页面路径，需http://格式的完整路径，不能加?id=123这类自定义参数，必须外网可以正常访问
                MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.return_url = successUrl;

                // 字符编码格式 目前支持 gbk 或 utf-8
                MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.input_charset = "utf-8";

                // 支付类型 ，无需修改
                MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.payment_type = "1";

                // 调用的接口名，无需修改
                MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.service = "create_direct_pay_by_user";
                ...
                {
                    {"service", MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.service},
                    {"partner", MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.partner},
                    {"seller_id", MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.seller_id},
                    {"_input_charset", MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.input_charset.ToLower()},
                    {"payment_type", MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.payment_type},
                    {"notify_url", MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.notify_url},
                    {"return_url", MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.return_url},
                    {"anti_phishing_key", MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.anti_phishing_key},
                    {"exter_invoke_ip", MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Config.exter_invoke_ip},
                ...
                //建立请求
                return MaiDar.Core.ThirdParty.Alipay.MApi.Pc.Submit.BuildRequest(sParaTemp, "get", "确认");
            }
        ...
        public string ChargeByAlipayMobi(string productName, decimal amount, string orderNo, string successUrl)
        {
            ...
            {
                // 合作身份者ID，签约账号，以2088开头由16位纯数字组成的字符串，查看地址：https://b.alipay.com/order/pidAndKey.htm
                MaiDar.Core.ThirdParty.Alipay.MApi.Mobi.Config.partner = config.AlipayMobiPid;

                // 收款支付宝账号，以2088开头由16位纯数字组成的字符串，一般情况下收款账号就是签约账号
                MaiDar.Core.ThirdParty.Alipay.MApi.Mobi.Config.seller_id = MaiDar.Core.ThirdParty.Alipay.MApi.Mobi.Config.partner;

                // MD5密钥，安全检验码，由数字和字母组成的32位字符串，查看地址：https://b.alipay.com/order/pidAndKey.htm
                MaiDar.Core.ThirdParty.Alipay.MApi.Mobi.Config.key = config.AlipayMobiMd5;

                // 服务器异步通知页面路径，需http://格式的完整路径，不能加?id=123这类自定义参数,必须外网可以正常访问
                MaiDar.Core.ThirdParty.Alipay.MApi.Mobi.Config.notify_url = string.Empty;

                // 页面跳转同步通知页面路径，需http://格式的完整路径，不能加?id=123这类自定义参数，必须外网可以正常访问
                MaiDar.Core.ThirdParty.Alipay.MApi.Mobi.Config.return_url = successUrl;

                //把请求参数打包成数组
                var sParaTemp = new SortedDictionary<string, string>
                {
                    {"partner", MaiDar.Core.ThirdParty.Alipay.MApi.Mobi.Config.partner},
                    {"seller_id", MaiDar.Core.ThirdParty.Alipay.MApi.Mobi.Config.seller_id},
                    {"_input_charset", MaiDar.Core.ThirdParty.Alipay.MApi.Mobi.Config.input_charset.ToLower()},
                    {"service", MaiDar.Core.ThirdParty.Alipay.MApi.Mobi.Config.service},
                    {"payment_type", MaiDar.Core.ThirdParty.Alipay.MApi.Mobi.Config.payment_type},
                    {"notify_url", MaiDar.Core.ThirdParty.Alipay.MApi.Mobi.Config.notify_url},
                    {"return_url", MaiDar.Core.ThirdParty.Alipay.MApi.Mobi.Config.return_url},
                    {"out_trade_no", orderNo},
                    {"subject", productName},
                    {"total_fee", amount.ToString("N2")},
                    {"show_url", successUrl},
                ...
                //建立请求
                return MaiDar.Core.ThirdParty.Alipay.MApi.Mobi.Submit.BuildRequest(sParaTemp, "get", "确认");
            }
