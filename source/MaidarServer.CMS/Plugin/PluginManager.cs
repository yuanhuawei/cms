...
namespace MaiDarServer.CMS.Plugin
{
    /// <summary>
    ...
    public static class PluginManager
    {
        ...
        public static bool ActiveAndAdd(PluginMetadata metadata, IPlugin plugin)
        {
            ...
            if (!string.IsNullOrEmpty(contentTable?.ContentTableName) && contentTable.ContentTableColumns != null && contentTable.ContentTableColumns.Count > 0)
            {
                ...
                if (!MaiDarDataProvider.DatabaseDao.IsTableExists(tableName))
                {
                    MaiDarDataProvider.TableCollectionDao.Delete(tableName);
                    MaiDarDataProvider.TableMetadataDao.Delete(tableName);
                    MaiDarDataProvider.TableStyleDao.Delete(tableName);

                    MaiDarDataProvider.TableCollectionDao.Insert(new AuxiliaryTableInfo(tableName,
                    ...
                    {
                        MaiDarDataProvider.TableMetadataDao.Insert(tableMetadataInfo);
                    }

                    MaiDarDataProvider.TableMetadataDao.CreateAuxiliaryTable(tableName);
                }
                else
                {
                    var columnNameList = MaiDarDataProvider.DatabaseDao.GetColumnNameList(tableName, true);
                    foreach (var tableColumn in contentTable.ContentTableColumns)
                    {
                    ...
                    MaiDarDataProvider.TableCollectionDao.UpdateIsChangedAfterCreatedInDb(false, tableName);
                }
            ...
            if (table?.Tables != null)
            {
                foreach (var tableName in table.Tables.Keys)
                {
                    ...
                    if (!MaiDarDataProvider.DatabaseDao.IsTableExists(tableName))
                    {
                        MaiDarDataProvider.DatabaseDao.CreatePluginTable(metadata.Id, tableName, tableColumns);
                    }
                    else
                    {
                        MaiDarDataProvider.DatabaseDao.AlterPluginTable(metadata.Id, tableName, tableColumns);
                    }
                }
            }
