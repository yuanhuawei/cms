    ...
    public class TagUtils
    {
        ...
        public static void AddTags(StringCollection tags, int publishmentSystemId, int contentId)
        {
            if (tags.Count > 0)
            {
                ...
                {
                    var tagInfo = MaiDarDataProvider.TagDao.GetTagInfo(publishmentSystemId,PageUtils.FilterXss(tagName));
                    if (tagInfo != null)
                    {
                    ...
                    else
                    {
                        ...
                        MaiDarDataProvider.TagDao.Insert(tagInfo);
                    }
                }
            }
        }
        ...
        public static void UpdateTags(string tagsLast, string tagsNow, StringCollection tagCollection, int publishmentSystemId, int contentId)
        {
            if (tagsLast != tagsNow)
            {
                ...
                foreach (string tag in tagsList)
                {
                    ...
                    {
                        var tagInfo = MaiDarDataProvider.TagDao.GetTagInfo(publishmentSystemId, tag);
                        if (tagInfo != null)
                        {
                            var contentIdList = TranslateUtils.StringCollectionToIntList(tagInfo.ContentIdCollection);
        ...
        {
            var tagInfoList = MaiDarDataProvider.TagDao.GetTagInfoList(publishmentSystemId, contentId);
