    ...
    public class AdminManager
    {
        ...
        public static AdministratorInfo GetAdminInfo(string userName)
        {
            ...
            lock (LockObject)
            {
                ...
                if (adminInfo == null)
                {
                    adminInfo = MaiDarDataProvider.AdministratorDao.GetByUserName(userName);
        ...
        public static bool CreateAdministrator(AdministratorInfo administratorInfo, out string errorMessage)
        {
            try
            {
                ...
                var isCreated = MaiDarDataProvider.AdministratorDao.Insert(administratorInfo, out errorMessage);
                if (isCreated == false) return false;

                var roles = new[] { EPredefinedRoleUtils.GetValue(EPredefinedRole.Administrator) };
                MaiDarDataProvider.AdministratorsInRolesDao.AddUserToRoles(administratorInfo.UserName, roles);
        ...
        public static string GetRolesHtml(string userName)
        {
            ...
            var roles = MaiDarDataProvider.AdministratorsInRolesDao.GetRolesForUser(userName);
