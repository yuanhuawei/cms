...
namespace MaiDar.Core.Provider
{
    public class TableCollectionDao : DataProviderBase
	{
			...
			using (var conn = GetConnection()) 
			{
				...
				using (var trans = conn.BeginTransaction()) 
				{
					try 
					{
                        ...
                        MaiDarDataProvider.TableMetadataDao.InsertSystemItems(info.TableEnName, info.AuxiliaryTableType, trans);
		...
		public void UpdateAttributeNum(string tableEnName)
		{
            ...
            var fieldNum = MaiDarDataProvider.TableMetadataDao.GetTableMetadataCountByEnName(tableEnName);
		...
		{
            var isTableExists = MaiDarDataProvider.DatabaseDao.IsTableExists(tableEnName);
			...
			using (var conn = GetConnection()) 
			{
				...
				using (var trans = conn.BeginTransaction()) 
				{
					try 
					{
                        ...
                        MaiDarDataProvider.TableMetadataDao.Delete(tableEnName, trans);
						TableManager.IsChanged = true;
						trans.Commit();
					}
					catch
		...
		public int GetTableUsedNum(string tableEnName, EAuxiliaryTableType tableType)
		{
            ...
            if (tableType == EAuxiliaryTableType.BackgroundContent)
            {
                ...
                try
                {
                    count += MaiDarDataProvider.DatabaseDao.GetIntResult(sqlString);
                }
                catch
                {
                    // ignored
            ...
            try
            {
                count += MaiDarDataProvider.DatabaseDao.GetIntResult(sqlString2);
            }
		    catch
		    {
		        // ignored
        ...
        public void CreateAllAuxiliaryTableIfNotExists()
        {
            ...
            if (!IsTableExists(EAuxiliaryTableType.BackgroundContent))
            {
                ...
                MaiDarDataProvider.TableCollectionDao.Insert(tableInfo);
                MaiDarDataProvider.TableMetadataDao.CreateAuxiliaryTable(tableInfo.TableEnName);
            }
        }
	}
}
