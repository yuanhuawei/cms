...
namespace MaiDar.Core.Provider
{
    public class AdministratorDao : DataProviderBase
    {
        ...
        private void Insert(AdministratorInfo info)
        {
            ...
            MaiDarDataProvider.DepartmentDao.UpdateCountOfAdmin();
            MaiDarDataProvider.AreaDao.UpdateCountOfAdmin();
        }
        ...
        public string GetSelectCommand(string searchWord, string roleName, int dayOfLastActivity, bool isConsoleAdministrator, string creatorUserName, int departmentId, int areaId)
        {
            ...
            if (!string.IsNullOrEmpty(roleName))
            {
                ...
                whereString =
                    $"WHERE (UserName IN (SELECT UserName FROM {MaiDarDataProvider.AdministratorsInRolesDao.TableName} WHERE RoleName = '{PageUtils.FilterSql(roleName)}')) {whereString}";
            }
            else
            {
                if (whereBuilder.Length > 0)
        ...
        public ArrayList GetUserNameArrayList(int departmentId, bool isAll)
        {
            ...
            if (isAll)
            {
                var departmentIdList = MaiDarDataProvider.DepartmentDao.GetDepartmentIdListForDescendant(departmentId);
